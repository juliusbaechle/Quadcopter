...
ProcessVars Sensor::read() {
  const float K = 0.001; // influence acc : gyro
  float ts = ((float) (micros() - m_us)) / 1000000;
  m_us = micros();
  
  Coordinates rates = readRates() - m_offset_rates;
  m_angles.x += ts * rates.x;
  m_angles.y += ts * rates.y;
  ...
  vars.speed_z += ts * a.acceleration.z;
  return vars;
}
